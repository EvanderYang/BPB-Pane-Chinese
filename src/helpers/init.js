...
export function initializeParams(request, env) {
    const proxyIPs = env.代理?.split(',').map(proxyIP => proxyIP.trim());
    ...
    if (pathName !== '/secrets') {
        ...
        if (typeof env.数据库 !== 'object') throw new Error('KV Dataset is not properly set! Please refer to tutorials.', { cause: "init"});
    }
}