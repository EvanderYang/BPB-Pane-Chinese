...
export async function getDataset(request, env) {
    ...
    try {
        proxySettings = await env.数据库.get("proxySettings", {type: 'json'});
        warpConfigs = await env.数据库.get('warpConfigs', {type: 'json'});
    } catch (error) {
        console.log(error);
...
export async function updateDataset (request, env) {
    ...
    if (!isReset) {
        try {
            currentSettings = await env.数据库.get("proxySettings", {type: 'json'});
        } catch (error) {
            console.log(error);
    ...
    try {    
        await env.数据库.put("proxySettings", JSON.stringify(proxySettings));
